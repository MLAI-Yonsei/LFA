...
def load_pretrained_model(cfg, model_name):
    ...
    model.load_model(
        f"/path/to/output_dir/{CUSTOM_DATASETS[cfg.DATASET.NAME]}/CLIP_Adapter/mom_lr35e-3_B4_ep40_16shots/seed1",
        epoch=40) # NOTE!!!
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, pretrained_model):
        super().__init__()
        # NOTE: CLIP-Adapter
        self.adapter = pretrained_model.adapter
        # NOTE: LFA
    ...
    def forward(self, image):
        ...
        ratio = 0.2
        x = self.adapter(image_features.type(torch.float32))
...
class LFAonCLIP_Adapter(TrainerX):
    ...
    def build_model(self):
        ...
        pretrained_model = load_pretrained_model(cfg, 'CLIP_Adapter')
