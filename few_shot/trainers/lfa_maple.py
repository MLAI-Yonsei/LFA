...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'LFAonMaPLe',
                      "vision_depth": 0,
...
def load_pretrained_model(cfg, model_name):
    ...
    if 'imagenet' in _dataset:
        model.load_model(
            f"/path/to/output_dir/imagenet/MaPLe/vit_b16_c2_ep5_batch4_2ctx_cross_datasets_16shots/seed1",
            epoch=2) # NOTE!!!
    else:
        model.load_model(
            f"/path/to/output_dir/{_dataset}/MaPLe/vit_b16_c2_ep5_batch4_2ctx_16shots/seed1",
            epoch=5) # NOTE!!!
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        # encoder_image
        x = self.image_encoder(image, shared_ctx, deep_compound_prompts_vision) # 12, batch_size, 768
        x = self.dropout(x)
        image_features, image_Q = self.image_encoder.self_attention(x)   # batch_size, 512
        
        x = self.text_encoder(prompts, deep_compound_prompts_text)
