...
def _hparams(algorithm, dataset, random_seed):
    ...
    elif algorithm in ["ZSCLIP", "LPCLIP", "CLIP_QLoRA"]:
    ...
    if dataset in SMALL_IMAGES or algorithm in ["DPLCLIP", "ZSCLIP", "LPCLIP", "CLIP_QLoRA", "CLIP_LFA", "CLIP_QLoRA_LFA"]:  # DPLCLIP , "DPLCLIP_with_captions"using SGD follower prior work.
        _hparam('lr', 1e-3, lambda r: 10**r.uniform(-4.5, -2.5))
    else:
        _hparam('lr', 5e-5, lambda r: 10**r.uniform(-5, -3.5))

    if dataset in SMALL_IMAGES or algorithm in ["DPLCLIP", "ZSCLIP", "LPCLIP", "CLIP_QLoRA", "CLIP_LFA", "CLIP_QLoRA_LFA"]:
