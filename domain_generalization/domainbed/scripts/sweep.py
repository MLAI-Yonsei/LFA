...
class Job:
    ...
    def __init__(self, train_args, sweep_output_dir):
        ...
        command = ["OMP_NUM_THREADS=1", "python3", "-m", "domainbed.scripts.train"]
...
class SAJob:
    ...
    def __init__(self, train_args, sweep_output_dir, ft_mode):
        ...
        command = [
            "python3",
            "-m",
            "domainbed.scripts.supervised_adaptation",
            "--input_dir",
        ...
        if os.path.exists(os.path.join(self.output_dir, "done")):
            ...
            os.path.join(self.output_dir, "results_{}.jsonl".format(ft_mode))
        ):
...
class UAJob:
    ...
    def __init__(self, train_args, sweep_output_dir, adapt_algorithm):
        ...
        command = [
            "python3",
            "-m",
            "domainbed.scripts.unsupervised_adaptation",
            "--input_dir",
            self.train_args["output_dir"],
            "--adapt_algorithm",
            adapt_algorithm,
        ...
        if os.path.exists(os.path.join(self.output_dir, "done")):
            if os.path.exists(
                os.path.join(self.output_dir, "done_{}".format(adapt_algorithm))
            ):
                self.state = UAJob.DONE
            else:
                self.state = UAJob.PRETRAINED
        elif os.path.exists(
            os.path.join(self.output_dir, "results_{}.jsonl".format(adapt_algorithm))
        ):
        ...
        for dataset in dataset_names:
            for algorithm in algorithms:
                ...
                for test_envs in all_test_envs:
                    for hparams_seed in range(
                        n_hparams_from, n_hparams_from + n_hparams
                    ):
                        train_args = {}
                        ...
                        train_args["find_hparams"] = find_hparams
                        train_args["test_hparams"] = test_hparams
                        train_args["activate_layer_lpclip"] = activate_layer_lpclip
                        train_args["load_pretrained"] = load_pretrained
                        if steps is not None:
                            train_args["steps"] = steps
                        if hparams is not None:
                            train_args["hparams"] = hparams
                        if feature_store_dir is not None:
                            train_args["feature_store_dir"] = feature_store_dir
                        if rank is not None:
...
if __name__ == "__main__":
    ...
    args_list = make_args_list(
        ...
        find_hparams=args.find_hparams,
        test_hparams=args.test_hparams,
        load_pretrained=args.load_pretrained,
        activate_layer_lpclip=args.activate_layer_lpclip,
    ...
    if args.command in ["launch", "delete_incomplete"]:
        ...
        jobs = [
            SAJob(train_args, args.output_dir, ft_mode="clf")
            for train_args in args_list
        ]
        jobs += [
            SAJob(train_args, args.output_dir, ft_mode="token")
            for train_args in args_list
        ]
        jobs += [
            SAJob(train_args, args.output_dir, ft_mode="transformer")
            for train_args in args_list
        ]
        jobs += [
            SAJob(train_args, args.output_dir, ft_mode="all")
            for train_args in args_list
        ]
        ...
        for job in jobs:
            print(job)
        print(
            "{} jobs: {} done, {} pretrained, {} incomplete, {} not launched.".format(
                len(jobs),
